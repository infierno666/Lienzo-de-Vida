...
// Íconos de Lucide React, ya que 'react-icons/fa' no se resuelve en este entorno.
import { Eye, EyeOff, Lock, Mail, LogIn, Globe, AlertCircle } from 'lucide-react'; 
import { Link, useNavigate } from 'react-router-dom';

// 💡 CREDENCIALES FIJAS PARA LA SIMULACIÓN (Actualizadas según tu referencia)
const DEMO_EMAIL = "admin@lienzodevida.com";
const DEMO_PASSWORD = "password123";
const TARGET_ROUTE = '/admin'; // Redirige a /admin
const USER_ROLE = 'admin'; // El rol que se guarda en localStorage

export default function Login() {
  // Manejamos la navegación de forma segura
  ...
  const [loading, setLoading] = useState(false); // Mantener el estado de carga
  
  const emailRef = useRef(null);

  useEffect(() => {
    // Foco en el campo de email al cargar
  ...
  const handleSubmit = async (e) => {
    ...
    // --- VALIDACIÓN DE CLIENTE (Mantenida) ---
    if (!email || !password) {
    ...
    // 1. Simular un delay de red (800ms)
    await new Promise((r) => setTimeout(r, 800));

    // --- LÓGICA DE AUTENTICACIÓN ACTUALIZADA (Verificación y Guardado de Rol) ---
    if (email === DEMO_EMAIL && password === DEMO_PASSWORD) {
      
      // 2. ÉXITO: Guardar estado de sesión Y el ROL en localStorage
      ...
      // 4. FALLO: Mostrar mensaje de error específico
      setError("Credenciales incorrectas. Solo administradores.");
      setLoading(false); // Quitar el estado de carga en caso de fallo
    }
  };
      ...
      <div className="w-full max-w-6xl bg-transparent grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <aside className="hidden lg:flex lg:col-span-7 rounded-xl overflow-hidden relative shadow-2xl">
          <img
            ...
            aria-hidden
            onerror="this.onerror=null;this.src='https://placehold.co/800x600/1e293b/ffffff?text=Imagen+Placeholder'"
          />
